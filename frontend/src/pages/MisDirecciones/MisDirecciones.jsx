    ...
    useEffect(() => {
        const cargarDatos = async () => {
            try {
                ...
                // Crear un mapa { id_region: descripcion }
                const regiones = {};
                for (const r of regionesData) {
                    regiones[r.id_region] = r.descripcion;
                }

                // Obtener comunas solo de las regiones necesarias
                const comunas = {};
    ...
    return (
        <div className="main-container">
            ...
            <div className="group-container">
                {direcciones?.length ? (
                    direcciones.map((dir) => (
                        <div key={dir.id_direccion} className="card-container">
                            <div>
                                <strong>{dir.direccion}</strong> Nº {dir.numero}
                                {dir.anexo && `, ${dir.anexo}`} <br />
                                Región: {regionesMap[dir.id_region] || "Desconocida"} – Comuna: {comunasMap[dir.id_comuna] || "Desconocida"}
                            </div>
