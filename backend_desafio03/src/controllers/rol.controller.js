import { createRolModel, readRolModel, updateRolModel } from "../models/rol.model.js"
...
export const updateRol = async (req, res) => {
    try {
        ...
        const result = await updateRolModel(descripcion, estado)
        res.status(200).json({ user: result })
    } catch (error) {
        res.status(500).json({ error: 'Error al procesar la solicitud' })
